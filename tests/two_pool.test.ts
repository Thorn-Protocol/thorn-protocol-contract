...
describe("StableSwapTwoPool Contract Tests", function () {
    ...
    before(async function () {
        ...
        swap_BUSD_USDC = (await ethers.getContractAt(
            "StableSwapTwoPool",
            info.swapContract
        ...
        poolInfoSC = (await poolInfoSC_CF.deploy(
            twoPoolInfoSC.address,
            threePoolInfoSC.address
    ...
    it("should add liquidity", async function () {
        ...
        await expect(swap_BUSD_USDC.add_liquidity([0, 1e6], 0)).to.be.reverted; //Initial deposit requires all coins

        await expect(swap_BUSD_USDC.add_liquidity([1e6, 0], 0)).to.be.reverted; //Initial deposit requires all coins

        await expect(swap_BUSD_USDC.add_liquidity([1e6, 1e6], 2e7)).to.be.reverted; //Slippage screwed you
    ...
    it("should add one coin into liquidity", async function () {
        ...
        let realAddToken0 = BigNumber.from(token0_balance_after.toString()).sub(
            BigNumber.from(token0_balance_before.toString())
        );
        let realLiquidityToken0AdminFee = BigNumber.from(defaultToken0Amount.toString()).sub(
            BigNumber.from(realAddToken0.toString())
        );
        let realLiquidityToken1AdminFee = BigNumber.from(token1_balance_before.toString()).sub(
            BigNumber.from(token1_balance_after.toString())
        );
        // check admin fee
    ...
    it("Remove liquidity imbalance", async () => {
        ...
        let liquidityAdminFee = await twoPoolInfoSC.get_remove_liquidity_imbalance_fee(swap_BUSD_USDC.address, [
            defaultTokenAmount,
            defaultTokenAmount,
        ]);
        ...
        let tx = await swap_BUSD_USDC.remove_liquidity_imbalance(
        ...
        assert.equal(
            defaultTokenAmount.toString(),
            (user_token0_balance_after - user_token0_balance_before).toString()
        );
        assert.equal(
            defaultTokenAmount.toString(),
    ...
    it("Swap token0 to token1", async () => {
        ...
        let cal_dx_amount = await poolInfoSC.get_dx(swap_BUSD_USDC.address, 0, 1, expect_token1_balance, 1e3);
        //check get_dx
