...
import { ERC20, StableSwapFactory, StableSwapInfo, StableSwapLPFactory, StableSwapRouter, StableSwapThreePool, StableSwapThreePoolDeployer, StableSwapThreePoolInfo, StableSwapTwoPool, StableSwapTwoPoolDeployer, StableSwapTwoPoolInfo, Token, WNATIVE } from "../typechain-types";
...
describe("test router", function () {
    ...
    before(async () => {
        ...
        stableSwap2Pool_ROSE_stROSE = await ethers.getContractAt("StableSwapTwoPool", info_2.swapContract) as unknown as StableSwapTwoPool;
        ...
        console.log("add liquidity success");
         ...
         stableSwap2Pool_ROSE_pROSE = await ethers.getContractAt("StableSwapTwoPool", info_1.swapContract) as unknown as StableSwapTwoPool;
         ...
         await (await stableSwap2Pool_ROSE_pROSE.add_liquidity([ADD_AMOUNT, ADD_AMOUNT],0,{ value: ADD_AMOUNT })).wait();
    ...
    it("swap ROSE to stROSE and vice versa", async () => {
        ...
        await expect(
            stableSwapRouter.exactInputStableSwap([ROSE,stROSE.address], [2],ADD_AMOUNT_1, 0,accounts[0].address, { value: ADD_AMOUNT_1.div(2) })
        ).to.be.reverted;

        await(await stableSwapRouter.exactInputStableSwap([ROSE,stROSE.address], [2],ADD_AMOUNT_1, 0,accounts[0].address, { value: ADD_AMOUNT_1 })).wait();
