...
async function add_liquidity_tethUSDT_ThornUSD() {
    ...
    const tethUSDT = ERC20__factory.connect(TOKEN_ADDRESS.tethUSDT, hre.ethers.provider);
    const ThornUSD = ERC20__factory.connect(TOKEN_ADDRESS.ThornUSD, hre.ethers.provider);
    const poolDeployment = await get("pool_tethUSDT-thornUSD");
    ...
    if (balance_lp < parseUnits("100", 18)) {
        ...
        console.log(" appove tethUSDT");
        const balance = await tethUSDT.balanceOf(deployer);
        console.log("balance tethUSDT", formatUnits(balance, 6));
        txRespone = await tethUSDT.connect(deployer2).approve(poolDeployment.address, amount);
        txReceipt = await txRespone.wait();

        console.log(" appove ThornUSD");
        const balance2 = await ThornUSD.balanceOf(deployer);
        console.log("balance ThornUSD", formatUnits(balance2, 6));
