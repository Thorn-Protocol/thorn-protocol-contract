...
async function add_liquidity_tethUSDT_ThornUSD() {
    ...
    const poolDeployment = await get("pool_bitUSDs-USDT");
    ...
    if (balance_lp < parseUnits("1", 18)) {
        ...
        const balance2 = await USDT.balanceOf(deployer);
        console.log("balance USDCe", formatUnits(balance2, 6));
        txRespone = await USDT.connect(deployer2).approve(poolDeployment.address, amountB);
        txReceipt = await txRespone.wait();
