...
interface IStableSwapTwoPoolInfo {
     ...
     * @param _swap Address of the stable swap contract
     */
    function balances(address _swap) external view returns (uint256[2] memory swapBalances);

    /**
     * @notice Get the exchange rates for each coin in the pool
     * @param _swap Address of the stable swap contract
     */
    function RATES(address _swap) external view returns (uint256[2] memory swapRATES);

    /**
     * @notice Get the precision multipliers for each coin in the pool
     * @param _swap Address of the stable swap contract
     */
    function PRECISION_MUL(address _swap) external view returns (uint256[2] memory swapPRECISION_MUL);
     ...
     */
    function calc_coins_amount(address _swap, uint256 _amount) external view returns (uint256[2] memory);
    ...
    */
     ...
     */
    function get_add_liquidity_mint_amount(address _swap, uint256[2] memory amounts)
        external
        view
        returns (uint256);
     ...
     * @param amounts Array of amounts for each coin being deposited
     */
    function get_add_liquidity_fee(address _swap, uint256[2] memory amounts)
        external
        view
        returns (uint256[2] memory liquidityFee);
     ...
     */
    function get_remove_liquidity_imbalance_fee(address _swap, uint256[2] memory amounts)
        external
        view
        returns (uint256[2] memory liquidityFee);
    ...
    */
    function _xp_mem(address _swap, uint256[2] memory _balances)
        external
        view
